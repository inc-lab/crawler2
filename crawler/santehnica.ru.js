...
module.exports = (async function(array) {
    ...
    try {
        ...
        await page.goto("https://santehnica.ru/",{timeout: 300000});
        ...
        if (array[i] != "") {
            ...
            try {
              ...
              //  console.log($('div[itemprop="offers"]').html());
                ...
                // Перечеркнутая цена
                let priceAdd =  (  $('.c-product-price-old').text()  ).toString().replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' );
                    priceAdd = priceAdd && !isNaN(priceAdd) ? parseInt(priceAdd) : false;    
                // Парсинг наличия
                let selector = $('.s-sale-sidebar__buy-btn.buy-buy').find('.ui-btn__text').text().toLowerCase();
