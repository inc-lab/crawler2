...
module.exports = (async function(array) {
    ...
    try {
        ...
        await page.goto("https://santehnika-online.ru/",{timeout: 300000});
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (array[i] != "") {
            ...
            try {
                ...
                //Цена на товар
                let price = $('.p-card-single-b-prices__result-price .b-price__price--main').find('meta[itemprop="price"]').attr('content');
                    price = price && !isNaN(price) ? parseInt(price) : false;
                    //console.log('s-online:'+price);
                   // console.log($('.card-product-bar-stick--top').html());
                ...
                // Перечеркнутая цена
                let priceAdd =  (  $('.b-price__price--old').find('.b-price__price-core').text()  ).toString().replace(/[^.\d]+/g,"").replace( /^([^\.]*\.)|\./g, '$1' );
                ...
                // Парсинг наличия
                let selector = $('.p-card-single-b-prices__button').find('.b-simple-button__text').text().toLowerCase();
